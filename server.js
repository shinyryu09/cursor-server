...
async function executeCursorCLI(command, workspacePath = null) {
    return new Promise(async (resolve, reject) => {
        ...
        if (command.includes('echo') && (command.includes('{') || command.includes('[') || command.includes('```'))) {
            try {
                // 프롬프트 추출
                const promptMatch = command.match(/echo "([^"]+)" \| cursor agent/);
                if (promptMatch) {
                    const prompt = promptMatch[1];
                    const tempFile = `/tmp/cursor_prompt_${Date.now()}.txt`;
                    fs.writeFileSync(tempFile, prompt, 'utf8');
                    finalCommand = `cat "${tempFile}" | cursor agent --print --output-format text --force`;
...
app.get('/v1/models', (req, res) => {
    // Xcode에서 전송하는 헤더 확인
    const xcodeProjectPath = req.headers['x-xcode-project-path'] || 
                            req.headers['x-project-path'] || 
                            req.headers['x-workspace-path'];
    
    if (xcodeProjectPath) {
        console.log(`Xcode project path detected: ${xcodeProjectPath}`);
        currentWorkspacePath = xcodeProjectPath;
    }
...
app.post('/v1/completions', async (req, res) => {
    try {
        ...
        if (xcodeProjectPath) {
            console.log(`Xcode project path detected in completions: ${xcodeProjectPath}`);
            currentWorkspacePath = xcodeProjectPath;
        }
        
        const { prompt, model = "cursor-cli", max_tokens = 1000, temperature = 0.7, stream = false, projectPath, workspacePath } = req.body;
        
        // 요청 본문에서도 프로젝트 경로 확인
        if (projectPath || workspacePath) {
            const detectedPath = projectPath || workspacePath;
            console.log(`Project path detected in request body: ${detectedPath}`);
            currentWorkspacePath = detectedPath;
        }
        ...
        if (promptStr.includes('{') && promptStr.includes('}')) {
            ...
            command = `cat "${tempFile}" | cursor agent --print --output-format text --force`;
        } else {
            command = `echo "${koreanPrompt}" | cursor agent --print --output-format text --force`;
        }
        let result;
        let responseContent;
...
app.post('/v1/chat/completions', async (req, res) => {
    try {
        ...
        if (xcodeProjectPath) {
            console.log(`Xcode project path detected in chat: ${xcodeProjectPath}`);
            currentWorkspacePath = xcodeProjectPath;
        }
        
        const { messages, model = "cursor-cli", max_tokens = 1000, temperature = 0.7, stream = false, projectPath, workspacePath } = req.body;
        ...
        if (isFileModificationRequest) {
            ...
            try {
                ...
                if (gameViewPath) {
                    ...
                    const modifyCommand = `echo "다음 요청에 한글로 답변하고 GameView.swift 파일을 실제로 수정해주세요: ${userMessage}" | cursor agent --print --output-format text --force`;
...
app.post('/chat', async (req, res) => {
    try {
        ...
        // Cursor CLI 명령어 구성
        const command = `cursor agent "${message}" --print --output-format text`;
...
app.post('/modify-file', async (req, res) => {
    try {
        ...
        const command = `echo "${instruction}" | cursor agent --print --output-format text --force`;
        let result;
        let responseContent;
